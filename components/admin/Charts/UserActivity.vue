...
// Selected metric
type MetricKey = 'activeUsers' | 'logins' | 'pageViews' | 'productViews' | 'cartAdditions'
const selectedMetric = ref<MetricKey>('activeUsers')

// Metric configurations
const metricConfig: Record<MetricKey, { label: string, color: string, backgroundColor: string }> = {
  activeUsers: {
    label: 'Active Users',
...
const chartData = computed((): ChartData => {
  ...
  return {
    ...
    datasets: [
      {
        label: config.label,
        data: trends.map(item => Number(item[selectedMetric.value] || 0)),
    ...
    tooltip: {
      callbacks: {
        title: (context) => {
          ...
          const date = new Date(props.data?.activityTrends?.[dataIndex ?? 0]?.date || '')
          return date.toLocaleDateString('en-US', {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
...
const averageDaily = computed(() => {
  ...
  const trends = props.data.activityTrends
  const total = trends.reduce((sum, item) => sum + Number(item[selectedMetric.value] || 0), 0)
...
const peakDay = computed(() => {
  ...
  const trends = props.data.activityTrends
  const maxValue = Math.max(...trends.map(item => Number(item[selectedMetric.value] || 0)))
...
const totalActivity = computed(() => {
  ...
  const trends = props.data.activityTrends
  return trends.reduce((sum, item) => sum + Number(item[selectedMetric.value] || 0), 0)
})
</script>
