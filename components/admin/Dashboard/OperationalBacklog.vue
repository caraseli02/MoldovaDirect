<template>
  <section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
    <header class="flex items-center justify-between">
      <h3 class="text-base font-medium text-gray-900">Operational Backlog</h3>
    ...
    <div v-if="loading && !items.length" class="mt-6 space-y-4">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div class="space-y-2">
            <div class="h-3 w-40 rounded-full bg-gray-200" />
            <div class="h-3 w-24 rounded-full bg-gray-100" />
          </div>
        </div>
        <div class="h-4 w-12 rounded-full bg-gray-200" />
      </div>
    </div>
      ...
      >
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="text-right">
            ...
            <p v-if="item.cta" class="text-xs font-medium text-blue-500 group-hover:text-blue-600">{{ item.cta }}</p>
          </div>
        </div>
      </component>
    </div>

    <div v-else class="mt-8 rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-6 text-center">
      <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-blue-50 text-blue-500">
        <commonIcon name="lucide:sparkles" class="h-6 w-6" />
      </div>
      <p class="mt-4 text-base font-medium text-gray-900">All clear — no pending workstreams</p>
