<template>
  <section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
    <header class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
      <div>
        <h3 class="text-base font-medium text-gray-900">Revenue & Customer Momentum</h3>
      ...
      <div class="mt-6 space-y-6">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div class="rounded-2xl border border-gray-100 bg-gray-50/70 p-4">
            <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Revenue Today</p>
            <div class="mt-2 flex items-baseline gap-2">
              <span :class="['text-xl', summaryDirectionColor(revenueSummary.direction)]">{{ directionEmoji(revenueSummary.direction) }}</span>
              <p class="text-base font-semibold text-gray-900">{{ revenueSummary.value }}</p>
            </div>
            <p class="text-sm text-gray-400">{{ revenueSummary.caption }}</p>
            <p class="mt-2 text-sm font-medium" :class="summaryDirectionColor(revenueSummary.direction)">
              {{ revenueSummary.delta }} vs avg
            </p>
          </div>
          <div class="rounded-2xl border border-gray-100 bg-gray-50/70 p-4">
            <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Customer Activity</p>
            <div class="mt-2 flex items-baseline gap-2">
              <span :class="['text-xl', summaryDirectionColor(customerSummary.direction)]">{{ directionEmoji(customerSummary.direction) }}</span>
              <p class="text-base font-semibold text-gray-900">{{ customerSummary.value }}</p>
            </div>
            <p class="text-sm text-gray-400">{{ customerSummary.caption }}</p>
            <p class="mt-2 text-sm font-medium" :class="summaryDirectionColor(customerSummary.direction)">
