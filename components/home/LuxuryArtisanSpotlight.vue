  ...
  <section class="py-16 md:py-24 bg-white">
    <div class="container mx-auto px-4 md:px-6">
      ...
      <div class="text-center max-w-3xl mx-auto mb-8 md:mb-12 lg:mb-16 px-4">
        ...
        <p class="text-base text-[var(--brand-dark)]/70 leading-relaxed mx-auto text-sm md:text-base">
      ...
      <div class="md:hidden mb-8 relative">
        ...
        >
          ...
          >
            <div class="border border-[var(--brand-dark)]/10 rounded-sm p-6 h-full">
              <!-- Portrait -->
              ...
              <!-- Content -->
              <div>
                <h3 class="font-serif text-xl font-semibold text-[var(--brand-accent)] mb-2">
                  {{ artisan.name }}
                </h3>

                <p class="text-sm uppercase tracking-wider text-[var(--brand-dark)] mb-4 font-semibold">
                  {{ artisan.specialty }}
                </p>

                <p class="text-[var(--brand-dark)]/80 leading-relaxed mb-6 italic text-base">
                  "{{ artisan.quote }}"
                </p>

                <div class="flex items-center gap-3 text-sm text-[var(--brand-dark)]/60">
      ...
      <div class="hidden md:grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 px-4 lg:px-0">
        <div
          v-for="(artisan, index) in artisans"
          :key="artisan.id"
          class="border border-[var(--brand-dark)]/10 rounded-sm p-6 group"
        >
          <!-- Portrait -->
          ...
          <!-- Content -->
          <div>
            <h3 class="font-serif text-xl md:text-2xl font-semibold text-[var(--brand-accent)] mb-2">
              {{ artisan.name }}
            </h3>

            <p class="text-xs sm:text-sm uppercase tracking-wider text-[var(--brand-dark)] mb-3 md:mb-4 font-semibold">
              {{ artisan.specialty }}
            </p>
            ...
            <div class="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-[var(--brand-dark)]/60">
              <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
              </svg>
              <span>{{ artisan.location }}</span>
      ...
      <div class="text-center mt-8 md:mt-12 lg:mt-16 px-4">
        <NuxtLink to="/producers" class="bg-[var(--brand-accent)] text-white px-8 py-3.5 text-sm font-medium uppercase tracking-wider hover:bg-[#8B3A47] transition-colors duration-300 bg-[var(--brand-dark)] text-[var(--brand-light)] px-8 py-3.5 text-sm font-medium uppercase tracking-wider hover:bg-[var(--brand-accent)] transition-colors duration-300 w-full sm:w-auto min-h-[48px] inline-flex items-center justify-center text-sm md:text-base">
...
.artisan-card::before {
  ...
  width: 100%;
  height: 3px;
  background: linear-gradient(to right, var(--brand-dark), var(--brand-accent));
  transform: scaleX(0);
