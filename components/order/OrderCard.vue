<template>
  ...
  >
    ...
    <div class="flex justify-between items-start mb-3">
      <div>
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
          <time :datetime="order.createdAt" :aria-label="$t('orders.accessibility.orderDate', { date: formatDate(order.createdAt) })">
    ...
    <!-- Order Items Preview -->
    <div class="mb-3" role="region" :aria-label="$t('orders.items')">
      <div v-if="previewItems.length > 0" class="flex items-center space-x-2">
        <div 
        ...
        <div v-if="remainingItemsCount > 0" class="text-xs text-gray-500 dark:text-gray-400" aria-live="polite">
    ...
    <!-- Order Summary -->
    <div class="flex justify-between items-center mb-3 pb-3 border-b border-gray-100 dark:border-gray-700" role="region" :aria-label="$t('common.orderSummary')">
      <span class="text-sm text-gray-600 dark:text-gray-400" :aria-label="$t('orders.accessibility.itemQuantity', { quantity: totalItems })">
        {{ $t('orders.totalItems') }}: {{ totalItems }}
      </span>
      <span class="text-base font-bold text-gray-900 dark:text-white" :aria-label="$t('orders.accessibility.orderTotal', { amount: formatPrice(order.totalEur) })">
