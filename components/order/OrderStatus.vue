<template>
  <div>
    ...
    >
      <span class="w-2 h-2 rounded-full mr-2" :class="dotClasses" aria-hidden="true"></span>
      {{ statusLabel }}
    </div>

    <!-- Progress Timeline (optional) -->
    <div v-if="showTimeline && timeline" class="mt-4">
      <nav class="relative" :aria-label="$t('orders.accessibility.trackingTimeline')">
        <!-- Timeline Line -->
        ...
        <ol class="space-y-4">
          ...
          >
            ...
            >
              ...
              <div v-else class="w-3 h-3 rounded-full bg-gray-300 dark:bg-gray-600" aria-hidden="true"></div>
            </div>
            ...
            <div class="ml-4 flex-1">
              ...
              <p v-if="event.timestamp" class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                <time :datetime="event.timestamp">{{ formatTimestamp(event.timestamp) }}</time>
              </p>
              <p v-if="event.description" class="text-xs text-gray-600 dark:text-gray-400 mt-1">
                {{ event.description }}
              </p>
            </div>
          </li>
      ...
      <!-- Estimated Delivery -->
      <div v-if="estimatedDelivery" class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg" role="region" :aria-label="$t('orders.estimatedDelivery')">
        <div class="flex items-center">
          <svg class="w-5 h-5 text-blue-600 dark:text-blue-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <div>
            ...
            <p class="text-sm font-semibold text-blue-600 dark:text-blue-400">
              <time :datetime="estimatedDelivery" :aria-label="$t('orders.accessibility.estimatedDelivery', { date: formatDate(estimatedDelivery) })">
