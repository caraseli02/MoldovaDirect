<template>
  <div id="main-content" class="space-y-10" role="main">
    <MobileVirtualProductGrid v-if="isMobile && products.length > 20" :items="products" :container-height="600"
      :loading="loading" @load-more="$emit('loadMore')" />
    ...
    <div v-if="pagination.totalPages > 1" class="space-y-4 text-center">
      ...
      <nav class="flex items-center justify-center gap-2" aria-label="Pagination">
        <UiButton :disabled="pagination.page <= 1" variant="outline" size="sm" class="rounded-full"
          :aria-label="t('products.pagination.previousPage')" @click="$emit('goToPage', pagination.page - 1)">
          {{ t('common.previous') }}
        </UiButton>
        <template v-for="page in visiblePages" :key="`page-${page}`">
          <UiButton v-if="isValidPage(page)" size="sm" :variant="page === pagination.page ? 'default' : 'ghost'"
            class="rounded-full" :class="page === pagination.page ? 'shadow-lg shadow-blue-500/30' : ''"
            :aria-label="t('products.pagination.goToPage', { page })"
            :aria-current="page === pagination.page ? 'page' : undefined" @click="$emit('goToPage', page)">
            {{ page }}
          </UiButton>
          <span v-else class="px-3 py-2 text-sm text-gray-600" aria-hidden="true">…</span>
        </template>
        <UiButton :disabled="pagination.page >= pagination.totalPages" variant="outline" size="sm" class="rounded-full"
          :aria-label="t('products.pagination.nextPage')" @click="$emit('goToPage', pagination.page + 1)">
          {{ t('common.next') }}
        </UiButton>
      </nav>
    </div>
