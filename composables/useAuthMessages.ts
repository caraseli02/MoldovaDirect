...
export const useAuthMessages = () => {
  ...
  const getPasswordResetInstructionsMessage = (): AuthMessage => {
    return createInfoMessage('auth.messages.passwordResetInstructions', 'reset')
  }
   ...
   */
  const getSessionExpiredMessage = (includeSecurityNotice = false): AuthMessage => {
    const message = includeSecurityNotice
      ? `${t('auth.messages.sessionExpired')} ${t('auth.messages.securityNotice')}`
