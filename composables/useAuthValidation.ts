...
export const useAuthValidation = () => {
  ...
  const nameSchema = z.string()
    ...
    .regex(/^[a-zA-ZÀ-ÿ\u0100-\u017F\u0400-\u04FF\s'-]+$/, 'auth.validation.name.invalid')
  ...
  const phoneSchema = z.string()
    .optional()
    .refine((val) => !val || /^[\+]?[1-9][\d]{0,15}$/.test(val), 'auth.validation.phone.invalid')
  ...
  const registerSchema = z.object({
    ...
    acceptTerms: z.boolean().refine(val => val === true, 'auth.validation.terms.required'),
   ...
   */
  const validateForm = <T>(schema: z.ZodSchema<T>, data: unknown): ValidationResult => {
    try {
      schema.parse(data)
      return {
        isValid: true,
