...
All SQL scripts live in the `supabase/sql/` directory. Run them in order via the Supabase SQL Editor or the Supabase CLI (`supabase db push` supports `--file`).

1. **Core schema** – `supabase/sql/supabase-schema.sql`
...
2. **Checkout and payments** – `supabase/sql/supabase-checkout-schema.sql` and `supabase/sql/supabase-checkout-indexes.sql`
   - Adds checkout-specific tables, indexes, and helper functions.
3. **Order tracking and returns** – `supabase/sql/supabase-order-tracking-schema.sql`, `supabase/sql/supabase-order-returns-schema.sql`, `supabase/sql/supabase-order-indexes.sql`, `supabase/sql/supabase-order-policies.sql`
...
4. **Analytics and support add-ons** – run any of the optional scripts your feature work requires:
   ...
   - `supabase/sql/supabase-cart-analytics-simple.sql`
   - `supabase/sql/supabase-support-tickets-schema.sql`
   - `supabase/sql/supabase-user-addresses-schema.sql`
   - `supabase/sql/supabase-add-reorder-point.sql`
   - `supabase/sql/supabase-mock-orders.sql` (seed data for demos; remove before production)
...
- Use `supabase/sql/supabase-mock-orders.sql` to create sample orders for the admin dashboard.
- When building new features, create dedicated seed scripts and store them alongside the other `supabase/sql/supabase-*.sql` files so the team can reproduce your data set.
...
- Internal SQL helpers – `supabase/sql/supabase-*.sql` files in the repo
