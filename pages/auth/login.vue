...
async function handleLogin(): Promise<void> {
  ...
  try {
    ...
    if (!localError.value) {
      localError.value = err instanceof Error ? err.message : t('auth.loginError')
    }
  }
  finally {
    loading.value = false
...
async function handleMagicLink(): Promise<void> {
  ...
  try {
    ...
    localError.value = err instanceof Error ? err.message : t('auth.magicLinkError')
  }
  finally {
...
async function handleSocialLogin(provider: 'google' | 'apple'): Promise<void> {
  ...
  try {
    ...
    localError.value = err instanceof Error ? err.message : t('auth.socialLoginError', { provider: provider.charAt(0).toUpperCase() + provider.slice(1) })
    loadingSocial.value = null
  }
}
