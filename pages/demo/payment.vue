      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
              <pre class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ JSON.stringify(checkoutStore.paymentMethod, null, 2) }}</pre>
            </div>
            
            <div class="mt-4 space-y-2">
              <div class="flex items-center justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400">Can Proceed:</span>
                <span :class="checkoutStore.canProceedToReview ? 'text-green-600' : 'text-red-600'">
                  {{ checkoutStore.canProceedToReview ? 'Yes' : 'No' }}
                </span>
              </div>
              <div class="flex items-center justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400">Current Step:</span>
                <span class="text-blue-600">{{ checkoutStore.currentStep }}</span>
              </div>
              <div class="flex items-center justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400">Processing:</span>
                <span :class="checkoutStore.processing ? 'text-yellow-600' : 'text-gray-600'">
                  {{ checkoutStore.processing ? 'Yes' : 'No' }}
                </span>
              </div>
            </div>
          </div>
        ...
        <div class="mt-8 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
          ...
          <div class="mt-4 p-4 bg-white dark:bg-gray-800 rounded border">
            <h4 class="font-medium text-gray-900 dark:text-white mb-2">To Enable Online Payments:</h4>
