        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
              <pre class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ JSON.stringify((checkoutStore as unknown).paymentMethod, null, 2) }}</pre>
            </div>

            <div class="mt-4 space-y-2">
              <div class="flex items-center justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400">Can Proceed:</span>
                <span :class="(checkoutStore as unknown).canProceedToReview ? 'text-green-600' : 'text-red-600'">
                  {{ (checkoutStore as unknown).canProceedToReview ? 'Yes' : 'No' }}
                </span>
              </div>
              <div class="flex items-center justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400">Current Step:</span>
                <span class="text-blue-600">{{ (checkoutStore as unknown).currentStep }}</span>
              </div>
              <div class="flex items-center justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400">Processing:</span>
                <span :class="(checkoutStore as unknown).processing ? 'text-yellow-600' : 'text-gray-600'">
                  {{ (checkoutStore as unknown).processing ? 'Yes' : 'No' }}
                </span>
              </div>
            </div>
          </div>
