    ...
    if (!targetTemplate) {
      ...
      for (const key of sourceKeys) {
        ...
        if (typeof sourceValue === 'object') {
          ...
          newTranslations[key] = `[${targetLocale.toUpperCase()}] ${sourceValue}`
        }
      }
      ...
      await supabase
        ...
        .insert({
          ...
          preheader: sourceTemplate.preheader ? `[${targetLocale.toUpperCase()}] ${sourceTemplate.preheader}` : null,
          version: 1
        })
    } else {
      ...
      for (const key of sourceKeys) {
        if (targetTemplate.translations[key] !== undefined) {
          ...
          if (typeof sourceValue === 'object') {
            ...
            mergedTranslations[key] = `[${targetLocale.toUpperCase()}] ${sourceValue}`
          }
        }
      }
