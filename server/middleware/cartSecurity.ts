  ...
  return defineEventHandler(async (event) => {
    ...
    try {
      ...
      if (!options.skipRateLimit && options.rateLimitOperation) {
        const rateLimit = checkRateLimit(rateLimitKey, options.rateLimitOperation as any)
        ...
        if (!rateLimit.allowed) {
          ...
          setHeader(event, 'Retry-After', Math.ceil((rateLimit.resetTime! - Date.now()) / 1000).toString())
