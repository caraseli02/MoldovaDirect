...
export const useCartStore = defineStore("cart", {
  ...
  getters: {
    ...
    itemCount: (state): number => {
      ...
      if (
        state._cachedItemCount &&
        state._lastItemsHash === state._currentItemsHash
      ) {
    ...
    getSavedForLaterItem:
      (state) =>
      (itemId: string): SavedForLaterItem | undefined => {
        return state.savedForLater.find((item) => item.id === itemId);
      },

    isProductSavedForLater:
      (state) =>
      (productId: string): boolean => {
        return state.savedForLater.some(
  ...
  actions: {
    ...
    getCachedValidation(
      productId: string
    ): { isValid: boolean; product: Product } | null {
    ...
    createDebouncedValidation() {
      ...
      self.debouncedValidateProduct = function (
        productId: string,
        delay: number = 1000
      ) {
        ...
        timeoutId = setTimeout(async () => {
          ...
          await self.batchValidateProducts(productIds);
        }, delay);
      };
    },
    ...
    async batchValidateProducts(productIds: string[]): Promise<void> {
      ...
      if (validationPromises.length > 0) {
        try {
          ...
          toastStore.warning(
            t("cart.validation.partial"),
            t("cart.validation.someProductsFailed")
          );
        }
      }
    },
    ...
    async validateSingleProduct(productId: string): Promise<void> {
      ...
      try {
        ...
        if (originalPrice !== currentProduct.price) {
          ...
          const { t } = useStoreI18n();
          toastStore.info(
            t("cart.notification.priceUpdated"),
            t("cart.notification.priceChange", {
        ...
        if (currentProduct.stock === 0) {
          ...
          const { t } = useStoreI18n();
          toastStore.warning(
            t("cart.notification.outOfStock"),
            t("cart.notification.removedOutOfStock", {
              product: cartItem.product.name,
            })
          );
          ...
          const { t } = useStoreI18n();
          toastStore.warning(
            t("cart.notification.quantityAdjusted"),
            t("cart.notification.quantityChange", {
              product: cartItem.product.name,
              oldQty: oldQuantity,
        ...
        if (error.statusCode === 404) {
          ...
          const { t } = useStoreI18n();
          toastStore.warning(
            t("cart.notification.productRemoved"),
            t("cart.notification.noLongerAvailable", {
              product: cartItem.product.name,
            })
          );
        } else {
          ...
          if (queueItem) {
            ...
            if (queueItem.retryCount >= 3) {
              ...
              const { t } = useStoreI18n();
              toastStore.error(
                t("cart.error.validationFailed"),
                t("cart.error.couldNotValidate", {
                  product: cartItem.product.name,
                })
              );
            }
    ...
    startBackgroundValidation() {
      if (!this.backgroundValidationEnabled || this.backgroundValidationWorker)
        return;
    ...
    async addItem(product: Product, quantity: number = 1) {
      ...
      try {
        ...
        if (cached && cached.isValid) {
          ...
          if (
            this.debouncedValidateProduct &&
            typeof this.debouncedValidateProduct === "function"
          ) {
            try {
        ...
        if (quantity > currentProduct.stock) {
          const { t } = useStoreI18n();
          const errorMsg = t("cart.error.onlyAvailable", {
            count: currentProduct.stock,
          });
        ...
        if (existingItem) {
          ...
          if (newQuantity > currentProduct.stock) {
            ...
            const errorMsg = t("cart.error.cannotAddMore", {
              quantity,
              available,
            });
            toastStore.error(t("cart.error.insufficientStock"), errorMsg, {
          ...
          const { t } = useStoreI18n();
          toastStore.success(
            t("cart.success.quantityUpdated"),
            t("cart.success.newQuantity", {
          ...
          const { t } = useStoreI18n();
          toastStore.success(
            t("cart.success.productAdded"),
            t("cart.success.addedToCart", { product: currentProduct.name })
          );
        }
    ...
    async secureAddItem(product: Product, quantity: number = 1) {
      ...
      try {
        ...
        const result = await cartSecurity.secureAddItem(
          product.id,
          quantity,
          this.sessionId
        );
        ...
        toastStore.success(
          t("cart.success.productAdded"),
          t("cart.success.addedToCart", { product: result.product.name })
        );
      } catch (error) {
    ...
    async secureUpdateQuantity(itemId: string, quantity: number) {
      ...
      try {
        ...
        // Use secure API endpoint
        const result = await cartSecurity.secureUpdateQuantity(
          itemId,
          quantity,
          this.sessionId
        );
          ...
          if (process.client) {
            try {
              ...
              if (cartAnalytics && cartAnalytics.trackQuantityUpdate) {
                cartAnalytics.trackQuantityUpdate(
                  item.product,
                  oldQuantity,
                  quantity,
                  this.subtotal,
        ...
        toastStore.success(
          t("cart.success.quantityUpdated"),
          t("cart.success.quantityChange", {
            product: item?.product.name,
            oldQty: item ? item.quantity : 0,
            newQty: quantity,
          })
        );
      } catch (error) {
    ...
    async secureRemoveItem(itemId: string) {
      ...
      try {
        ...
        // Client-side validation
        const validation = cartSecurity.validateCartData("removeItem", {
          itemId,
        });
          ...
          if (process.client) {
            try {
              ...
              if (cartAnalytics && cartAnalytics.trackRemoveFromCart) {
                cartAnalytics.trackRemoveFromCart(
                  removedItem.product,
        ...
        toastStore.success(
          ...
          t("cart.success.removedFromCart", { product: item.product.name }),
          {
            ...
            actionHandler: function () {
              ...
              toastStore.success(
                t2("cart.success.productRestored"),
                t2("cart.success.restoredToCart", {
                  product: item.product.name,
                })
              );
            }.bind(this),
    ...
    async secureClearCart() {
      ...
      try {
        ...
        const { t } = useStoreI18n();
        toastStore.success(
          t("cart.success.cartCleared"),
          t("cart.success.allItemsRemoved"),
          {
            actionText: t("common.undo"),
            actionHandler: function () {
              ...
              const { t: t2 } = useStoreI18n();
              toastStore.success(
                t2("cart.success.cartRestored"),
                t2("cart.success.itemsRestoredToCart")
              );
            }.bind(this),
            duration: 10000,
          }
    ...
    async secureValidateCart() {
      ...
      try {
        ...
        // Use secure API endpoint
        const result = await cartSecurity.secureValidateCart(
          items,
          this.sessionId
        );
            ...
            switch (validation.action) {
              case "remove":
                ...
                const index = this.items.findIndex(
                  (item) => item.id === validation.itemId
                );
                if (index !== -1) {
      ...
      try {
        ...
        if (quantity > currentProduct.stock) {
          ...
          toastStore.error(t("cart.error.insufficientStock"), errorMsg, {
            ...
            actionHandler: () =>
              this.updateQuantity(itemId, currentProduct.stock),
          });
          throw new Error(errorMsg);
        }
    ...
    async removeItem(itemId: string) {
      ...
      try {
        ...
        toastStore.success(
          t("cart.success.productRemoved"),
          t("cart.success.removedFromCart", {
    ...
    createDebouncedSave() {
      ...
      self.debouncedSaveToStorage = function () {
        if (timeoutId) {
          clearTimeout(timeoutId);
        }
    ...
    async initializeCartCaching() {
      ...
      try {
        const { initializeCartCache } = await import(
          "~/composables/useCartCache"
        );
    ...
    saveAndCacheCartData() {
      ...
      if (
        ...
        typeof this.debouncedSaveToStorage === "function"
      ) {
        try {
          this.debouncedSaveToStorage();
        } catch (saveError) {
    ...
    migrateCartData(
      fromStorage: "localStorage" | "sessionStorage",
      toStorage: "localStorage" | "sessionStorage"
    ): boolean {
      if (!process.client) return false;

      try {
        const storage =
          fromStorage === "localStorage" ? localStorage : sessionStorage;
        const targetStorage =
          toStorage === "localStorage" ? localStorage : sessionStorage;
        ...
        if (cartData) {
          ...
          if (validation.fixedData) {
            targetStorage.setItem(
              "moldova-direct-cart",
              JSON.stringify(validation.fixedData)
            );
            ...
            const { t } = useStoreI18n();
            toastStore.success(
              t("cart.success.cartMigrated"),
              t("cart.success.dataMigratedTo", { storage: toStorage })
            );
            return true;
          }
        }
        ...
        console.warn(
          `Failed to migrate cart data from ${fromStorage} to ${toStorage}:`,
          error
        );
        return false;
      }
    ...
    saveToStorage() {
      ...
      if (!validation.isValid && validation.fixedData) {
        console.warn(
          "Cart data was corrupted and has been fixed:",
          validation.errors
        );
      ...
      if (this.storageType === "localStorage") {
        try {
          ...
          try {
            ...
            const { t } = useStoreI18n();
            toastStore.warning(
              t("cart.warning.limitedStorage"),
              t("cart.warning.sessionOnlyStorage")
            );
          } catch (sessionError) {
            console.error(
            ...
            const { t } = useStoreI18n();
            toastStore.error(
              t("cart.error.storageError"),
              t("cart.error.changesWillBeLost")
            );
          }
        }
      ...
      if (this.storageType === "memory" && !saved) {
        const { t } = useStoreI18n();
        toastStore.warning(
          t("cart.warning.temporaryMode"),
          t("cart.warning.cannotSave")
        );
      }
    },
    ...
    async bulkRemoveSelected() {
      ...
      try {
        ...
        toastStore.success(
          ...
          t("cart.success.itemsRemovedCount", { count: removedItems.length }),
          {
            ...
            actionHandler: () => {
              ...
              const { t: t2 } = useStoreI18n();
              toastStore.success(
                t2("cart.success.productsRestored"),
                t2("cart.success.itemsRestoredToCart")
              );
            },
            duration: 8000,
        ...
        const { t } = useStoreI18n();
        toastStore.error(
          t("cart.error.bulkRemoveFailed"),
          t("cart.error.couldNotRemoveSelected")
        );
      } finally {
      ...
      try {
        ...
        if (updatedCount > 0) {
          ...
          toastStore.success(
            ...
            t("cart.success.productsUpdatedTo", {
              count: updatedCount,
              quantity: newQuantity,
            })
          );
        ...
        const { t } = useStoreI18n();
        toastStore.error(
          t("cart.error.bulkUpdateFailed"),
          t("cart.error.couldNotUpdateQuantities")
        );
      } finally {
        this.bulkOperationInProgress = false;
      }
    ...
    async bulkSaveForLater() {
      ...
      try {
        ...
        const { t } = useStoreI18n();
        toastStore.error(
          t("cart.error.saveForLaterFailed"),
          t("cart.error.couldNotSaveProducts")
        );
      } finally {
        this.bulkOperationInProgress = false;
      }
    ...
    async saveItemForLater(itemId: string) {
      ...
      try {
        ...
        toastStore.success(
          t("cart.success.savedForLater"),
          t("cart.success.productSavedForLater", {
            product: cartItem.product.name,
          }),
          {
            actionText: t("common.undo"),
        ...
        toastStore.error(
          t("cart.error.saveForLaterFailed"),
          t("cart.error.couldNotSaveProduct")
        );
      }
    },
    ...
    async moveFromSavedToCart(savedItemId: string) {
      ...
      try {
        ...
        if (existingItem) {
          ...
          if (newQuantity <= savedItem.product.stock) {
            ...
            toastStore.success(
              ...
              t("cart.success.newQuantity", {
                product: savedItem.product.name,
                quantity: newQuantity,
              })
            );
          } else {
            ...
            const availableQuantity =
              savedItem.product.stock - existingItem.quantity;
            if (availableQuantity > 0) {
              existingItem.quantity = savedItem.product.stock;
              const { t } = useStoreI18n();
              toastStore.warning(
                t("cart.warning.quantityAdjusted"),
                t("cart.warning.adjustedToMax", {
                  product: savedItem.product.name,
                  max: savedItem.product.stock,
                })
              );
            } else {
              const { t } = useStoreI18n();
          ...
          toastStore.success(
            t("cart.success.movedToCart"),
            t("cart.success.productMovedToCart", {
              product: savedItem.product.name,
            })
          );
        }
        ...
        const { t } = useStoreI18n();
        toastStore.error(
          t("cart.error.moveToCartFailed"),
          t("cart.error.couldNotMoveToCart")
        );
      }
    },
    ...
    async removeFromSavedForLater(savedItemId: string) {
      ...
      try {
        ...
        toastStore.success(
          t("cart.success.productRemoved"),
          t("cart.success.removedFromSaved", {
            product: removedItem.product.name,
          }),
          {
            actionText: t("common.undo"),
            actionHandler: () => {
              ...
              toastStore.success(
                t2("cart.success.productRestored"),
                t2("cart.success.restoredProduct", {
                  product: removedItem.product.name,
                })
              );
            },
        ...
        const { t } = useStoreI18n();
        toastStore.error(
          t("cart.error.removeFailed"),
          t("cart.error.couldNotRemoveSaved")
        );
      }
    },
      ...
      toastStore.success(
        ...
        t("cart.success.allSavedRemoved"),
        {
          ...
          actionHandler: () => {
            ...
            const { t: t2 } = useStoreI18n();
            toastStore.success(
              t2("cart.success.savedRestored"),
              t2("cart.success.savedItemsRestored")
            );
          },
          duration: 10000,
        }
    ...
    async loadRecommendations() {
      ...
      try {
        ...
        const categoryIds = [
          ...new Set(this.items.map((item) => item.product.categoryId)),
        ];
    ...
    loadFromStorage() {
      ...
      if (this.storageType === "memory") {
        const { t } = useStoreI18n();
        toastStore.warning(
          t("cart.warning.storageUnavailable"),
          t("cart.warning.temporaryMode")
        );
        this.sessionId = this.generateSessionId();
        return;
      ...
      try {
        ...
        if (savedCart && sourceStorage) {
          ...
          try {
            ...
            const { t } = useStoreI18n();
            toastStore.error(
              t("cart.error.dataCorrupted"),
              t("cart.error.cartDataReset")
            );
          ...
          if (validation.errors.length > 0) {
            ...
            const { t } = useStoreI18n();
            toastStore.warning(
              t("cart.warning.cartRepaired"),
              t("cart.warning.issuesDetectedAndFixed")
            );
          }
          ...
          if (validation.fixedData) {
            ...
            if (
              ...
              const { t } = useStoreI18n();
              toastStore.info(
                t("cart.info.cartExpired"),
                t("cart.info.cartCleanedDueToExpiry")
              );
              return;
            }
            ...
            if (typeof cartData.backgroundValidationEnabled === "boolean") {
              this.backgroundValidationEnabled =
                cartData.backgroundValidationEnabled;
            }

            if (cartData.lastBackgroundValidation) {
              try {
                this.lastBackgroundValidation = new Date(
                  cartData.lastBackgroundValidation
                );
              } catch {
            ...
            if (
              cartData.savedForLater &&
              Array.isArray(cartData.savedForLater)
            ) {
            ...
            if (
              sourceStorage === "sessionStorage" &&
              this.detectAvailableStorage() === "localStorage"
            ) {
            ...
            const { t } = useStoreI18n();
            toastStore.error(
              t("cart.error.invalidData"),
              t("cart.error.couldNotRecoverCart")
            );
          }
        } else {
          // No saved cart found
        ...
        const { t } = useStoreI18n();
        toastStore.error(
          t("cart.error.loadFailed"),
          t("cart.error.couldNotLoadSavedCart")
        );
      }
    },
    ...
    async recoverCartData(): Promise<boolean> {
      ...
      try {
        ...
        if (availableStorage !== this.storageType) {
          ...
          const { t } = useStoreI18n();
          toastStore.info(
            t("cart.info.storageChanged"),
            t("cart.info.storageChangedDetails", { storage: availableStorage })
          );
        }
        ...
        if (this.items.length > 0) {
          ...
          const { t } = useStoreI18n();
          toastStore.success(
            t("cart.success.cartRecovered"),
            t("cart.success.dataRecoveredAndValidated")
          );
          return true;
        }
      ...
      try {
        ...
        if (validationSummary.hasChanges) {
          if (validationSummary.removedItems.length > 0) {
            const { t } = useStoreI18n();
            toastStore.warning(
              t("cart.warning.productsUnavailable"),
              t("cart.warning.removedItems", {
                items: validationSummary.removedItems.join(", "),
              }),
              {
                duration: 8000,
              }
          ...
          if (validationSummary.adjustedItems.length > 0) {
            const { t } = useStoreI18n();
            toastStore.warning(
              t("cart.warning.quantitiesAdjusted"),
          ...
          const { t } = useStoreI18n();
          toastStore.success(
            t("cart.success.cartValidated"),
            t("cart.success.allProductsAvailable")
          );
        }
        ...
        toastStore.error(
          t("cart.error.validationFailed"),
          t("cart.error.couldNotValidateCart"),
          {
            actionText: t("common.retry"),
            actionHandler: () => this.validateCart(),
          }
