...
// Configure tests to run on mobile devices
const mobileConfig = {
  'iPhone 13': devices['iPhone 13'],
  'Pixel 5': devices['Pixel 5'],
}

for (const [deviceName, device] of Object.entries(mobileConfig)) {
  test.describe(`Mobile Responsive - ${deviceName}`, () => {
    test.use(device)
    ...
    test.describe('Orientation Changes', () => {
      test('should handle portrait to landscape orientation', async ({ page, context }) => {
        await page.goto('/auth/login')
