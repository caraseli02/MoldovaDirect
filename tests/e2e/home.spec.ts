...
test.describe('Home Page', () => {
  ...
  test('should display the main navigation', async ({ page, isMobile }) => {
    ...
    if (isMobile) {
      // On mobile, check that the header is present with logo
      const header = page.locator('header')
      await expect(header).toBeVisible()

      // Verify the logo link is visible in the header (be specific to avoid multiple matches)
      const logo = header.getByRole('link', { name: 'Moldova Direct' })
